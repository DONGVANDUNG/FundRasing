...
using esign.Authorization.Accounts;
...
using esign.Web.Models.Account;
using esign.Security;
using esign.Security.Recaptcha;
using esign.Sessions;
using esign.Url;
using esign.Web.Authentication.External;
using esign.Web.Security.Recaptcha;
using esign.Web.Session;
using esign.Web.Views.Shared.Components.TenantChange;
...
{
    public class AccountController : esignControllerBase
    {
            ...
            string returnUrlHash = "", string ss = "")
        {
            ...
            if (loginResult.User.ShouldChangePasswordOnNextLogin)
            {
                ...
                var expireDate =
                    Uri.EscapeDataString(Clock.Now.AddHours(1).ToString(esignConsts.DateTimeOffsetFormat));
