...
using esign.Migrations.Seed;
using esign.Migrations.Seed.Host;
...
namespace esign.Install
{
    [AbpAllowAnonymous]
    [DisableAuditing]
    public class InstallAppService : esignAppServiceBase, IInstallAppService
    {
        private readonly AbpZeroDbMigrator<esignDbContext> _migrator;
        ...
        {
            var connectionString = _appConfiguration[$"ConnectionStrings:{esignConsts.ConnectionStringName}"];
        ...
        {
            EditAppSettingsjson($"ConnectionStrings:{esignConsts.ConnectionStringName}", constring);
        }
